...
public class BaseTpnsBean implements Serializable {
	...
	public void goToProfilePage() {
		ExternalContext externalContext = FacesContext.getCurrentInstance().getExternalContext();
		try {
			externalContext.redirect(externalContext.getRequestContextPath() + "/pages/profile.xhtml");
		} catch (IOException e) {
	...
	public void goToCreateArticlePage() {
		ExternalContext externalContext = FacesContext.getCurrentInstance().getExternalContext();
		try {
			externalContext.redirect(externalContext.getRequestContextPath() + "/pages/editArticle.xhtml");
		} catch (IOException e) {
			LOGGER.error("Redirect to admin page failed", e);
		}
	}
